...
// Convierte una fila cruda de D1 en el objeto que consume el frontend,
...
function formatEntry(row) {
  ...
  // realmente cerrada (fecha + precio de salida cargados); si no, no se
  // muestra un resultado que todavía no es real.
...
export async function update(db, id, body) {
  const existing = await db.prepare('SELECT * FROM trade_journal WHERE id = ?').bind(id).first();
  if (!existing) throw { status: 404, message: 'Registro no encontrado' };
  ...
  const updated = await db.prepare('SELECT * FROM trade_journal WHERE id = ?').bind(id).first();
  return formatEntry(updated);
}

export async function remove(db, id) {
  const result = await db.prepare('DELETE FROM trade_journal WHERE id = ?').bind(id).run();
  if (result.meta.changes === 0) throw { status: 404, message: 'Registro no encontrado' };
}
